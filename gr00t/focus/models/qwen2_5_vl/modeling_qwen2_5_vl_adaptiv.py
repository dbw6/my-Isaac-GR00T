...
from gr00t.focus.utils import naive_scaled_dot_product_attention, TEXT_TOKEN
...
def Qwen2_5_VLDecoderLayer_adaptiv_forward(
    self,
...
def Qwen2_5_VLSdpaAttention_adaptiv_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def Qwen2_5_VLMLP_adaptiv_forward(self, x):
...
def Qwen2_5_VLForConditionalGeneration_adaptiv_forward(
    self,
    input_ids: torch.LongTensor = None,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    """
    Custom forward function for Qwen2.5 VL with adaptiv metadata recording.
    ...
    ### ADAPTIV METADATA CALCULATION START ###
    # Calculate metadata for focus before calling self.model()
    if hasattr(self, 'adaptiv') and input_ids is not None:
    ...
    ### ADAPTIV METADATA CALCULATION END ###
