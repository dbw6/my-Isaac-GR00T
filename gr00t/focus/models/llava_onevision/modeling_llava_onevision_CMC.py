...
def prepare_inputs_labels_for_multimodal_onevision_CMC(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
    ...
    assert image_token_length % patch_num == 0, f"Image token length {image_token_length} is not divisible by patch number {patch_num}."
    ...
    self.cmc.prepare(patch_type, patch_height, patch_width, image_token_start_index, image_token_end_index, image_token_length, original_length, new_input_embeds)
