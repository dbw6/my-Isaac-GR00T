...
def prepare_inputs_labels_for_multimodal_get_patch_type_onevision(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
            ...
            for image_idx, image_feature in enumerate(image_features):
                ...
                if image_idx in video_idx_in_batch:  # video operations
                    ...
                    if "maxpool2x2" in mm_patch_merge_type:
                        ...
                        # my_height, my_width = image_feature.shape[1:]
                        # # save height and width to output/attn/shape.txt, each time create a new file
                        # with open("output/attn/shape.txt", "w") as f:
                        #     f.write(f"{my_height}, {my_width}\n")
                        c, h, w = image_feature.shape
                        image_feature = image_feature.flatten(1, 2).transpose(0, 1)
                        ...
                        image_feature = unpad_image(image_feature, image_sizes[image_idx])
                        # my_height, my_width = image_feature.shape[1:]
                        # print("my_height, my_width", my_height, my_width)
                        image_feature = torch.cat((image_feature, self.model.image_newline[:, None, None].expand(*image_feature.shape[:-1], 1).to(image_feature.device)), dim=-1)
                        image_feature = image_feature.flatten(1, 2).transpose(0, 1)
                    else:
    ...
    if modalities[0] == "video":
        ...
        height_stride = patch_width
        width_stride = 1
        assert patch_height * patch_width + height * width == image_token_length
