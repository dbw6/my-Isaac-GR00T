...
# Get the directory containing this file for config path resolution
_FOCUS_DIR = os.path.dirname(os.path.abspath(__file__))

# Focus methods
from gr00t.focus.main import Focus
from gr00t.focus.baseline_CMC import CMC
from gr00t.focus.baseline_adaptiv import Adaptiv
from gr00t.focus.models.llava_video.modeling_llava_video import (
    prepare_inputs_labels_for_multimodal_get_patch_type,
)
from gr00t.focus.models.llava_onevision.modeling_llava_onevision import (
    prepare_inputs_labels_for_multimodal_get_patch_type_onevision,
)
from gr00t.focus.models.minicpmv.modeling_minicpmv import (
    get_vllm_embedding
)
from gr00t.focus.models.llava_video.modeling_llava_video_CMC import (
    prepare_inputs_labels_for_multimodal_CMC,
)
from gr00t.focus.models.llava_onevision.modeling_llava_onevision_CMC import (
    prepare_inputs_labels_for_multimodal_onevision_CMC,
)
from gr00t.focus.models.minicpmv.modeling_minicpmv_CMC import (
    get_vllm_embedding_CMC
)
from gr00t.focus.models.llava_video.modeling_llava_video_adaptiv import (
    prepare_inputs_labels_for_multimodal_get_patch_type_adaptiv,
)
from gr00t.focus.models.llava_onevision.modeling_llava_onevision_adaptiv import (
    prepare_inputs_labels_for_multimodal_get_patch_type_onevision_adaptiv,
)
from gr00t.focus.models.minicpmv.modeling_minicpmv_adaptiv import (
    get_vllm_embedding_adaptiv,
)

from gr00t.focus.models.qwen2.modeling_qwen2 import (
    Qwen2DecoderLayer_focus_forward,
    Qwen2Model_focus_forward,
    Qwen2SdpaAttention_focus_forward,
    Qwen2MLP_focus_forward
)
from gr00t.focus.models.qwen2.modeling_qwen2_CMC import (
    Qwen2DecoderLayer_CMC_forward,
    Qwen2Model_CMC_forward,
    Qwen2SdpaAttention_CMC_forward,
    Qwen2MLP_CMC_forward,
)
from gr00t.focus.models.qwen2.modeling_qwen2_adaptiv import (
    Qwen2DecoderLayer_adaptiv_forward,
    Qwen2Model_adaptiv_forward,
    Qwen2SdpaAttention_adaptiv_forward,
    Qwen2MLP_adaptiv_forward
)
from gr00t.focus.models.qwen2_5_vl.modeling_qwen2_5_vl import (
...
from gr00t.focus.models.qwen2_5_vl.modeling_qwen2_5_vl_adaptiv import (
...
# Qwen3 Focus support for GR00T integration
from gr00t.focus.models.qwen3.modeling_qwen3 import (
    Qwen3DecoderLayer_focus_forward,
    Qwen3Model_focus_forward,
    Qwen3Attention_focus_forward,
    Qwen3MLP_focus_forward
)
# Eagle3_VL Focus support for GR00T integration
from gr00t.focus.models.eagle3_vl.modeling_eagle3_vl import (
    Eagle3_VLForConditionalGeneration_focus_forward
)
from gr00t.focus.utils import get_attr_by_name
...
def apply_focus(model, cli_args=None):
    ...
    if LlavaQwenForCausalLM is not None and isinstance(model, LlavaQwenForCausalLM):
    ...
    # Eagle3_VL Model (GR00T's VLM backbone with Qwen3)
    elif hasattr(model, 'config') and hasattr(model.config, 'architectures') and model.config.architectures and 'Eagle3_VLForConditionalGeneration' in model.config.architectures[0]:
        if cli_args.focus:
            # Replace the main forward function with custom forward that includes metadata calculation
            model.forward = MethodType(Eagle3_VLForConditionalGeneration_focus_forward, model)
            # Eagle3_VL uses Qwen3 as LLM
            llm_forward = Qwen3Model_focus_forward
            decoder_forward = Qwen3DecoderLayer_focus_forward
            attention_forward = Qwen3Attention_focus_forward
            mlp_forward = Qwen3MLP_focus_forward
        else:
            raise NotImplementedError("Only focus is supported for Eagle3_VL models currently")
...
def replace_focus_forward(
    ...
    if cli_args.focus:
        if cli_args.similarity_threshold == -1.0:
            ...
            config = pd.read_csv(os.path.join(_FOCUS_DIR, "configs", "focus.csv"), skipinitialspace=True)
        ...
        if cli_args.CMC_threshold == -1.0:
            ...
            config = pd.read_csv(os.path.join(_FOCUS_DIR, "configs", "cmc.csv"), skipinitialspace=True)
            config_match = config[(config["model"] == cli_args.model) & (config["dataset"] == cli_args.tasks)]
            if config_match.empty:
                config_match = config[config["model"] == "default"]
        ...
        if cli_args.adaptiv_threshold == -1.0:
            ...
            config = pd.read_csv(os.path.join(_FOCUS_DIR, "configs", "adaptiv.csv"), skipinitialspace=True)
            config_match = config[(config["model"] == cli_args.model) & (config["dataset"] == cli_args.tasks)]
            if config_match.empty:
                config_match = config[config["model"] == "default"]
...
def get_model_dimensions(model, llm_key, decoder_key, mlp_key):
    ...
    Supports both standard models (config in model.config) and VLM models like Eagle3_VL 
    (config in model.config.text_config).
    ...
    # Determine which config to use (some VLMs like Eagle3_VL store LLM config in text_config)
    config = model.config
